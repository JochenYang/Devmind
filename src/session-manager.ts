...
import { Project, Session, SessionCreateParams, AiMemoryConfig } from './types.js';
...
export class SessionManager {
  ...
  private detectProjectType(projectPath: string): {
    ...
    // 检查配置文件
    const configFiles = [
      { file: 'package.json', language: 'javascript', parser: this.parsePackageJson },
      { file: 'pyproject.toml', language: 'python', parser: this.parsePyproject },
  ...
  private parsePackageJson(filePath: string): { framework?: string; metadata: Record<string, any> } {
    ...
    else if (dependencies.nest || dependencies['@nestjs/core']) framework = 'nestjs';
  ...
  private parsePyproject(filePath: string): { framework?: string; metadata: Record<string, any> } {
  ...
  private parseGoMod(filePath: string): { framework?: string; metadata: Record<string, any> } {
    const content = readFileSync(filePath, 'utf-8');
    const metadata: Record<string, any> = {};
  ...
  private parseCargoToml(filePath: string): { framework?: string; metadata: Record<string, any> } {
    const content = readFileSync(filePath, 'utf-8');
    const metadata: Record<string, any> = {};
    ...
    for (const configFile of configFiles) {
      ...
      if (existsSync(configPath)) {
        try {
          ...
          const configHash = createHash('md5').update(content).digest('hex').substring(0, 8);
  ...
  generateProjectHash(projectPath: string, gitRemoteUrl?: string): string {
    let hashInput = resolve(projectPath);
