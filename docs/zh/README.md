...
- **🤖 自动记忆** - 后台守护进程监控文件变化、Git操作和错误日志
- **🧠 语义搜索** - AI驱动的向量嵌入搜索,查找相关上下文
- **📝 持久存储** - 基于SQLite的本地存储,完全私密
- **🔍 混合搜索** - 结合关键词和语义搜索,获得最佳结果
- **⚡ 实时响应** - 开发过程中记录,即时检索
- **🛠️ 跨工具支持** - 兼容多个MCP客户端和开发环境
- **📂 智能项目索引** - 智能项目全局分析与自动特征提取
...
要启用智能自动记录,请将这些规则添加到您的MCP客户端系统提示中:

```text
使用DevMind MCP实现智能开发记忆:

• 回答技术问题前: 使用 semantic_search
• 用户说"记住这个": 使用 record_context
• bug修复/解决方案: 使用 record_context,type="solution"
• 决策/配置说明: 使用 record_context,type="documentation"
